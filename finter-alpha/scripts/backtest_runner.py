...
def validate_positions(positions):
    ...
    if max_sum > 1e8 + 1000:  # Allow small rounding error
        issues['errors'].append(
            f"Row sums exceed 1e8 (total AUM). Max: {max_sum:.0f}"
        )
    ...
    if (positions < 0).any().any():
        issues['warnings'].append(
            "Contains negative positions (short positions)"
        )
...
def run_backtest(alpha_file, start_date, end_date, universe, 
                commission=0.0025, slippage=0.001, tax=0.0023):
    """
    ...
    commission : float
        Commission rate (default: 0.0025 = 0.25%)
    slippage : float
        Slippage rate (default: 0.001 = 0.1%)
    tax : float
        Transaction tax rate (default: 0.0023 = 0.23%)
    """
    ...
    try:
        ...
        print(f"  ✓ Positions generated successfully")
    ...
    row_sums = positions.sum(axis=1)
    print(f"\n  Position Statistics:")
    print(f"    Row sum - Min: {row_sums.min():.0f}, "
...
Examples:
  ...
  python backtest_runner.py --code alpha.py --start 20200101 --end 20241231
  python backtest_runner.py --code alpha.py --universe us_stock
  python backtest_runner.py --code alpha.py --commission 0.005
        """
    )
    
    parser.add_argument(
        '--code', required=True,
        help='Path to Python file containing Alpha class'
    )
    
    parser.add_argument(
        '--start', type=int, default=20200101,
        help='Start date in YYYYMMDD format (default: 20200101)'
    )
    
    parser.add_argument(
        '--end', type=int, default=20241231,
        help='End date in YYYYMMDD format (default: 20241231)'
    )
    
    parser.add_argument(
        '--universe', default='kr_stock',
        choices=['kr_stock', 'us_stock'],
        help='Market universe (default: kr_stock)'
    )
    
    parser.add_argument(
        '--commission', type=float, default=0.0025,
        help='Commission rate (default: 0.0025 = 0.25%%)'
    )
    ...
    parser.add_argument(
        '--tax', type=float, default=0.0023,
        help='Transaction tax rate (default: 0.0023 = 0.23%%)'
    )
